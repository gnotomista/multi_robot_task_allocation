...
mqtt_interface = MqttInterface('robots', 'localhost', 1883, 10);
mqtt_interface.subscribe('allocation');
...
global task_exo_dist x_mud y_mud
load('data/mud')
t_endogenous = 10;
task_exo_dist = 2;
...
while true
    ...
    msg_send.x = x;
    msg_send.t = 0;
    mqtt_interface.send_json('robots_state', msg_send);
    msg_recv = mqtt_interface.receive_json('allocation');
    if isempty(msg_recv)
        continue
    end
    break
...
for iter = 1 : max_iter
    ...
    msg_send.x = x;
    msg_send.t = t;
    msg_send.scenario_params = updated_scenario_params;
    msg_send.specialization = S;
    mqtt_interface.send_json('robots_state', msg_send);
    msg_recv = mqtt_interface.receive_json('allocation');
    alpha = msg_recv.alpha;
    ...
    [u, delta, time_to_synthesize_controller, opt_sol_info] = rta.solve_reduced_qp(x, reshape(alpha,n_r*n_t,1), t);
    ...
    drawnow % limitrate % limitrate makes each step of the robot control loop faster, therefore solving the MIQP takes more steps ---> the difference between centralized and mixed centr/decentr is bigger
    
    print_info(t, time_to_synthesize_controller, task_assignment, u, delta, S, t_endogenous)
    avg_time_to_solve_QP = (avg_time_to_solve_QP*(iter-1)+time_to_synthesize_controller)/iter;
    disp(['time to solve QP: ', num2str(time_to_synthesize_controller)])
    disp(['average time to solve QP: ', num2str(avg_time_to_solve_QP)])
end
...
global task_exo_dist x_mud y_mud
cond = i ~= 5 && ... % not the quad ...
    alpha(task_exo_dist,i) > 0 && ... % ... is assigned to task task_exo_dist and ...
    inpolygon(x(1,i),x(2,i),x_mud,y_mud); % ... is in the mud
end
...
% plots
function h_quad = plot_quad(h_quad, x_toplot)
x = x_toplot(1,5,end);
y = x_toplot(2,5,end);
dir = x_toplot(:,5,end)-x_toplot(:,5,end-1);
